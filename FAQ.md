...
guide](https://github.com/hyz/Hex/blob/master/CONTRIBUTING.md).

### Why is the latest version not on F-Droid yet?

We don't release new versions of Hex on F-Droid ourselves. Once we've released
...
### Can you port Hex to iOS/Windows/MacOS/Browser Extension?

We don't have plans to port Hex to other platforms.
...
adding support for this feature in Hex.
...
If you could previously unlock Hex with biometrics, but suddenly can't do so
...
settings of Hex and re-enable it.

### Why does Hex keep prompting me for my password, even though I have enabled biometric authentication?

You're probably encountering the password reminder. Try entering your password
to unlock the vault once. After that, Hex will prompt for biometrics by
...
### Hex uses SHA1 for most/all of my tokens. Isn't that insecure?
...
were to change this on Hex' end, the tokens would stop working. Furthermore,
when using SHA1 in an HMAC calculation, the currently known issues in SHA1 are
not of concern.

### Why doesn't Hex support biometric unlock for my device, even though it works with other apps?

The reason for this is pretty technical. In short, since you're not entering
your password when using biometric unlock, Hex needs some other way to decrypt
...
resulting in the error displayed to you by Hex in an error dialog.

If biometrics works with other apps, but not with Hex, that means those other
apps probably perform a weaker form of biometric authentication.

## Backups

### How can I back up my Hex vault to the cloud automatically?

Hex can only automatically back up to the cloud if the app of your cloud
provider is installed on your device and fully participates in the Android
Storage Access Framework. Hex doesn't have access to the internet and we don't
...
Another common setup is to configure Hex to back up to a folder on local
...
Hex uses the same password you have configured to encrypt your vault as the
...
without a password and then importing that into Hex. Another option is
extracting the ZIP archive on a computer and importing the resulting
Accounts.txt file into Hex with the "Plain text" import option.
