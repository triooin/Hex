...
import com.hyz.hex.vectors.VaultEntries;
...
public class DatabaseImporterTest {
     ...
     *     -> while read line; do (qrencode "$line" -o - | feh -); done < ./app/src/test/resources/com/hyz/hex/importers/plain.txt
    ...
    @Test
    public void testImportHexPlain() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importPlain(HexImporter.class, "hex_plain.json");
        checkImportedEntries(entries);
    }

    @Test
    public void testImportHexEncrypted() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(HexImporter.class, "hex_encrypted.json", encryptedState -> {
            final char[] password = "test".toCharArray();
            return ((HexImporter.EncryptedState) encryptedState).decrypt(password);
        });
