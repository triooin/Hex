...
Issuer field in Hex is matched against for automatic selection of an icon for
...
### Using icon packs in Hex

Users can download an icon pack from the internet and import it into Hex
through the settings menu. Hex extracts the icon pack to
...
After that, Hex will start proposing icons for new entries if the issuer
