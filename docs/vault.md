...
Hex persists the user's token secrets and related information to a file. This
...
This document describes Hex' security design and file format. It's split up
...
Two cryptographic primitives were selected for use in Hex. An Authenticated
...
Hex vault also relies on the assumption that this limit is never exceeded.
This is a reasonable assumption to make, because it's highly unlikely that an
Hex user will ever come close to saving the vault 2<sup>32</sup> times.
...
Hex supports unlocking a vault with multiple different credentials. The main
...
vault](/app/src/test/resources/com/hyz/hex/importers/hex_plain.json)
and an [encrypted
vault](/app/src/test/resources/com/hyz/hex/importers/hex_encrypted.json)
are available in the [test
data](/app/src/test/resources/com/hyz/hex/importers) folder.
There's also an example Python script that can decrypt an Hex vault given the
...
encrypted and Hex will try to parse it as such.
...
the same as TOTP, but it uses a different final encoding step. Hex'
implementation of it can be found in
[crypto/otp/OTP.java](/app/src/main/java/com/hyz/hex/crypto/otp/OTP.java).
...
There is no specification available for MOTP. Hex' implementation of it can be
found in
[crypto/otp/MOTP.java](/app/src/main/java/com/hyz/hex/crypto/otp/MOTP.java).
...
There is no specification available for Yandex's OTP algorithm. Hex'
implementation can be found in
[crypto/otp/YAOTP.java](/app/src/main/java/com/hyz/hex/crypto/otp/YAOTP.java)
