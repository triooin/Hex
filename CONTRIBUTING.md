...
We use [Crowdin](https://crowdin.com/project/hex-authenticator) to crowdsource
translations of Hex for lots of different languages. __Pull requests that
...
proposal](https://github.com/hyz/Hex/issues/new?labels=proposal&template=feature.md)
...
typically only necessary if Hex:
...
3. Start a log capture by executing the following commands.
    ...
    capture the log output of Hex, replace the last logcat command with the
    one below:

    ```sh
    adb logcat --pid=$(adb shell pidof -s com.hyz.hex) > debug.log
    ```

    _If you are using a debug APK, replace ``com.hyz.hex`` with
   ``com.hyz.hex.debug``._
